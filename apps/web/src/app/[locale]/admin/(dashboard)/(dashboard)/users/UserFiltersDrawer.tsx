...
type Filters = Omit<UserParams, "page">;
...
const STATUS_OPTIONS: { value: UserParams["status"]; color: "standard" | "success" | "warning" }[] = [
    { value: "all", color: "standard" },
    { value: "active", color: "success" },
    { value: "noactive", color: "standard" },
    { value: "blocked", color: "warning" },
];

const SORT_BY_OPTIONS: UserParams["sortBy"][] = ["createdAt", "lastLoginAt", "lastSeenAt"];

export function UserFiltersDrawer({ open, onClose, filters, setFilter, resetFilters }: UserFiltersDrawerProps) {
            ...
            <Box sx={{ width: 300, p: 2 }} display="flex" flexDirection="column" gap={2}>
                ...
                <Box display="flex" flexDirection="column" gap={0.5}>
                    <StyledTypography variant="caption" color="text.secondary" fontWeight={600} sx={{ px: 0.5 }}>
                        {t("pages.admin.users.filter.sortBy")}
                    </StyledTypography>
                        ...
                        return (
                            ...
                            >
                                <StyledTypography variant="body2" color="inherit">
                                    {t(`pages.admin.users.filter.${opt}`)}
                                </StyledTypography>
                                {selected ? (
                                    filters.order === "desc" ? (
                    ...
                    >
                        {STATUS_OPTIONS.map(({ value, color }) => (
                            <ToggleButton key={value} value={value} color={color}>
                                {t(`pages.admin.users.status.${value}`)}
                            </ToggleButton>
                        ))}
                    </ToggleButtonGroup>
                </Box>
