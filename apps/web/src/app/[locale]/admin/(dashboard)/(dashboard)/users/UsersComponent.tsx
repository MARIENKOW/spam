...
import { useUsers, defaultUserParams } from "@/hooks/tanstack/useUsers";
import { usePageClamp } from "@/hooks/tanstack/usePageClamp";
import { UserList } from "@/app/[locale]/admin/(dashboard)/(dashboard)/users/UserList";
import { PaginationComponent } from "@/components/common/PaginationComponent";
import { SearchField } from "@/components/common/SearchField";
import { useUrlListState } from "@/hooks/tanstack/useUrlListState";
import { UserFiltersDrawer } from "@/app/[locale]/admin/(dashboard)/(dashboard)/users/UserFiltersDrawer";

export default function UsersComponent() {
    const t = useTranslations();
    const { page, setPage, filters, setFilter, resetFilters } = useUrlListState(defaultUserParams);
    const { data, isFetching, error, refetch } = useUsers({ page, ...filters });
    usePageClamp(page, data?.meta.pageCount, setPage);
    const [filtersOpen, setFiltersOpen] = useState(false);

    const activeFilterCount =
        (filters.status !== defaultUserParams.status ? 1 : 0) +
        (filters.sortBy !== defaultUserParams.sortBy ? 1 : 0);
    ...
    return (
        <Box display="flex" flexDirection="column" flex={1} height="100%">
            ...
            <Box flex={1} display="flex" flexDirection="column" position="relative" gap={2} py={2}>
                ...
                <UserList data={data?.data} error={error} />
                <PaginationComponent
                    page={page}
