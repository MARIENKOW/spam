import TgAccountsComponent from "@/app/[locale]/(admin)/(dashboard)/(dashboard)/tg-accounts/TgAccountsComponent";
...
import { defaultTgAccountParams } from "@/lib/tanstack/listDefaults";
...
const { getAll } = new TgAccountService($apiAdminServer);

interface Props {
    searchParams: Promise<unknown>;
}

export default async function TgAccountsPage({ searchParams }: Props) {
    const params = await searchParams;
    const parsed = parseListParams(params, defaultTgAccountParams);
    const queryKey = tgAccountKeys.list(parsed);
